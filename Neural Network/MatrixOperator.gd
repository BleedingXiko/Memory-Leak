...
static func normalize(a: Matrix):
	# Returns each weight as a probability of it contributing to the error
	var result = Matrix.new(a.rows, a.cols)
	var TotalSum = []
	for i in range(result.rows):
		var sum = 0
		for j in range(result.cols):
			sum = sum + abs(a.data[i][j])
		
		TotalSum.append(sum)
	
	for i in range(result.rows):
		for j in range(result.cols):
			result.data[i][j] = abs(a.data[i][j]) / TotalSum[i]
	return result
